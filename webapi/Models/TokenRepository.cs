...
namespace webapi.Models
{
    public class TokenRepository
    {
        ...
        public void AddForUser(string email, string tokentype, string jwttoken)
        {
            ...
            {
                dbConnection.Open();
                string gQuery = "SELECT * FROM [tbluser]"
                                + " WHERE email = @Email";
                var users = dbConnection.Query<User>(gQuery, new { Email = email });
                if (users.Any())
                {
                    int userid = users.SingleOrDefault().UserId;
        ...
        //public Token GetByTypeAndToken(string tokentype, int userId, string jwttoken)
        public Token GetByTypeAndToken(string tokentype, string jwttoken)
        {
            using (IDbConnection dbConnection = Connection)
            {
                //string sQuery = "SELECT * FROM [token] WHERE token_type = @Token_Type AND user_id = @USER_ID and substring(jwt_token, 1, 100) = @JWT_Token;";
                string sQuery = "SELECT * FROM [token] WHERE token_type = @Token_Type AND jwt_token = @JWT_Token;";
                dbConnection.Open();
                //var getToken = dbConnection.Query<Token>(sQuery, new { Token_Type = tokentype, USER_ID = userId,JWT_Token = jwttoken });
